...
router.post('/', (req, res) => {
  ...
  const query = 'INSERT INTO blog (blog_title , blog_desc, blog_text, blog_pic ,  start_date , end_date) VALUES (?, ?, ?, ?, ?)';
...
router.put('/:id', (req, res) => {
  const { blog_title , blog_desc, blog_pic , start_date , end_date } = req.body;
  const query = 'UPDATE blog SET blog_title=?, blog_desc=?, blog_text=?, blog_pic=? , start_date=? , end_date=? WHERE id_blog = ?';
